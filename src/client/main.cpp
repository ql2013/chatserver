                    ...
                    else 
                    {
                        ...
                        if(responsejs.contains("offlinemsg"))
                        {
                            ...
                            for(string &str:vec)
                            {
                                ...
                                if(ONE_CHAT_MSG==_js["msgid"].get<int>())
                                {
                                    cout<<_js["time"].get<string>()<<"["<<_js["id"]<<"]"<<_js["name"].get<string>()
                                        <<"said:"<<_js["msg"].get<string>()<<endl;
                                    
                                }
                               else  
                                {
                                    cout<<"群消息[:"<<_js["groupid"]<<"]:"<<_js["time"].get<string>()<<"["<<_js["id"]<<"]"<<_js["name"].get<string>()
                                        <<"said:"<<_js["msg"].get<string>()<<endl;
...
void mainMenu(int clientfd)
{
    ...
    while(isMainMenuRuning)
    {
        ...
        //调用相应的命令的事件处理回调，mainMenu对修改封闭,添加新功能不需要修改信息
...
void chat(int clientfd,string str)
{
    ...
    json js1;
    js1["msgid"]=ONE_CHAT_MSG;
    js1["id"]=g_currentUser.getId();
    js1["name"]=g_currentUser.getName();
    js1["to"]=friendid;
    js1["msg"]=message;
    js1["time"]=getCurrentTime();
    string buffer=js1.dump();
...
void creategroup(int clientfd,string str)
{
    ...
    if(idx==-1)
    {
        cerr<<"create command invaild!"<<endl;
        return ;
    }
    ...
    if(-1==len)
    {
        cerr<<"send creategroupt msg error ->"<<buffer<<endl;
    }
}
//"addgroup" command handler
void addgroup(int clientfd,string str)
...
string getCurrentTime()
{
    ...
    sprintf(date,"%d-%02d-%02d %02d:%02d:%02d",
            (int)ptm->tm_year+1900,(int)ptm->tm_mon+1,(int)ptm->tm_mday,(int)ptm->tm_hour,
