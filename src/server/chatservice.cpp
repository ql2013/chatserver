...
using namespace std;
//记录using 前缺少什么 原因 #include 文件出错，找寻对应文件中定义的类最后是不是少了一个;
...
void ChatService::login(const TcpConnectionPtr &conn,json &js,Timestamp time)
{
    ...
    if(user.getId()!=-1&&user.getPwd()==pwd)
    {
        if(user.getState()=="online")
        {
            ...
            response["errno"]=2;
            response["errmsg"]="该账号已经登陆,不能重复登录";
        ...
        {   
            //登录成功，记录用户连接信息
           {
            ...
            _redis.subscribe(id);
                ...
                //登录成功，更新用户状态信息 set offline=>online
